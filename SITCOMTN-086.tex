...
(PSF).  See Section~\ref{sec:bfecorr} for more information on the correction to
this effect.
...
charge that is left behind.  This is the ``global charge-transfer ineffeciency
(CTI)'' that is corrected by a model from Snyder and Roodman (2020).
...
more information on the CTI correction.
...
(see Section~\ref{sec:}).  We also assume that this is the location in the
signal chain where additional 2D bias structure including the bias ``wave''
(bias structure along the serial direction) is coming from.
...
The signal now travels down a set of cables from all the amplifiers into the
Readout Board and ASPIC.  It is in this cabling that the crosstalk signal
between neighboring amplifiers (which are in neighboring cables) appears.  This
crosstalk has some small non-linearity, as studied by Snyder (Thesis).  There
is additional crosstalk between amplifiers across the midline that is typically
of the opposite sign, and comes from ASPIC-to-ASPIC interactions.  We do not
...
The signal then is measured with a dual-slope integrator.  [Need somebody to
  write a couple of sentences here].  This has another effective gain ($g_3$)
that depends on the readout time and hence the time duration of the integration
per pixel.  This gain may also be temperature dependent, but that is completely
degenerate with the pre-amp temperature dependence.
...
photoelectron map as they originally hit the surface of the detector.  The
flowchart outlining our ISR pipeline is in Figure~\ref{fig:isr_model}.  We now
explain these steps and how they relate to the detector model described previously.
...
\label{sec:overscancorr}

We now run serial overscan correction to remove the overall bias level in each
amplifier. Due to the fact that many channels in LSSTCam are not completely
stable with time, our default mode is to perform row-by-row serial overscan
correction to remove this bias level as well as possible bias drifts and
jumps. We have 64 overscan pixels per row, and we have 64 overscan pixels per
row.  We take the median of each row, skipping the first 3 and final 3 columns
to avoid pixels with residual charge-transfer-inefficiency (CTI) which bleeds
into the serial overscan region.
...
the gain measurement from the PTC analysis [ref].  All downstream calibrations
are either produced in $\etron$ units, or are adjusted using the gain values to
ensure consistent application of crosstalk, linearity, CTI, etc.
...
also want to add an image of the electronics in question].  As discussed above,
this part of the signal chain is prior to the input to the ASPIC and therefore
the natural units are $\etron$.
...
a crosstalk signal into the other crosstalk regions.  That is why we perform
parallel overscan subtraction after the crosstalk correction.
...
that structured bias signal is properly removed.  One of the main assumptions
is that bleeds are always positive deviations in the parallel overscan region,
while significant negative deviations are primarily from bias structure.
...
% correction on just the parallel overscan region (see Section~\ref{sec:xtalkcorr}
...
transfer inefficiency (CTI).  The CTI model of Snyder and Roodman (2020) is
calibrated with the ``extended pixel edge response'' (EPER) measurement that
shows how much imaging charge from flat-field images leaks into the serial
overscan region.
...
We determine the dark current by taking a series of 30 second images (to match
the survey integration time) and taking a clipped mean combination after
rejecting obvious cosmic rays in the dark images.  The typical dark current is
small (XX electrons per second) but may be larger for some amplifiers.

\subsection{Brighter-Fatter Correction}
\label{sec:bfecorr}

Finally, we correct for the Brighter/Fatter effect.  To derive the BFE kernel
to correct this effect we make use of the fact that the BFE creates
pixel-to-pixel correlations in flat-field images.  The BFE kernel itself is
derived from the same flat-field pairs that are used to derive the PTC gain.
See Broughton et al. (2003) [ref] for details on our algorithm for estimating
the BFE kernel as well as correcting it in the pixel images.
